        ...
        <div className="max-w-7xl mx-auto flex flex-col gap-16">
          ...
          <div className="footer-reveal flex flex-col md:flex-row justify-between items-start md:items-end gap-8 pb-12">
            ...
            <div className="relative group">
              <a href={`mailto:${content.global.contactEmail}`} className="font-serif text-5xl md:text-7xl text-[#e6e4e2] hover:text-[#d4c3b3] transition-colors lowercase block relative" aria-label={`Send email to ${content.global.contactEmail}`}>
                {content.ui.contact}
                <span className="absolute left-0 top-1/2 w-full h-[3px] md:h-[4px] bg-[#d4c3b3] scale-x-0 origin-left transition-transform duration-500 ease-out group-hover:scale-x-100" />
              </a>
            </div>
          </div>
          ...
          <div className="footer-reveal grid grid-cols-2 md:grid-cols-4 gap-12 text-[10px] uppercase tracking-[0.2em] text-[#908e8b]">
            ...
            <nav className="flex flex-col gap-4" aria-label="Footer Sitemap">
              <span className="text-white/20 select-none">{content.ui.sitemap}</span>
              <ul className="flex flex-col gap-3 font-medium">
                <li>
                  <Link href="/" className="hover:text-white transition-colors">{content.ui.home}</Link>
                </li>
                <li>
                  <a href="#philosophy" className="hover:text-white transition-colors">{content.ui.philosophy}</a>
                </li>
                <li>
                  <Link href="/expertise" className="hover:text-white transition-colors">{content.ui.expertise}</Link>
                </li>
                <li>
                  <a href="#company" className="hover:text-white transition-colors">{content.ui.company}</a>
                </li>
              </ul>
            </nav>
            ...
            <nav className="flex flex-col gap-4" aria-label="Footer Services">
              <span className="text-white/20 select-none">{content.ui.services}</span>
              <ul className="flex flex-col gap-3 font-medium">
            ...
            <address className="flex flex-col gap-4 not-italic">
              <span className="text-white/20 select-none">{content.global.addresses.gujarat.title}</span>
              <div className="normal-case leading-relaxed font-light text-[11px]">
                <span className="uppercase text-[10px] tracking-[0.2em] font-medium block mb-2 text-[#e6e4e2]">{content.global.addresses.gujarat.line1}</span>
            ...
            <div className="flex flex-col gap-4 justify-between">
              <div className="flex flex-col gap-3">
                ...
                <ul className="flex flex-col gap-3">
                  ...
                  <li>
                    <a href={content.global.socials.instagram} target="_blank" rel="noopener noreferrer" className="hover:text-white transition-colors flex items-center gap-2" aria-label="Visit Instagram Profile (opens in new tab)">
                      instagram
                      <svg className="w-3.5 h-3.5 fill-current text-current opacity-70" viewBox="0 0 16 16" aria-hidden="true">
          ...
          <div className="footer-reveal flex flex-col md:flex-row justify-between items-start md:items-center gap-6 pt-8 text-[9px] uppercase tracking-[0.25em] text-[#908e8b]">
            ...
            <div className="flex flex-wrap gap-x-16 md:gap-x-24 gap-y-2 font-mono text-[10px]">
              <div className="flex items-center gap-2">
                ...
                <span className="text-[#908e8b] font-sans text-[8px] tracking-widest">{content.global.addresses.newDelhi.timezoneLabel}</span>
              </div>
              <div className="flex items-center gap-2">
                <span className="text-white">{newYorkTime}</span>
                <span className="text-[#908e8b] font-sans text-[8px] tracking-widest">{content.global.addresses.newYork.timezoneLabel}</span>
              </div>
            </div>
